...
import { roleElement } from '/@/api/demo/model/systemModel';
...
export function useDataSource(
  ...
  async function fetch(opt?: FetchParams) {
    ...
    try {
      ...
      const { current = 1, limit = PAGE_SIZE } = unref(getPaginationInfo) as PaginationProps;

      if ((isBoolean(pagination) && !pagination) || isBoolean(getPaginationInfo)) {
        pageParams = {};
      } else {
        pageParams[pageField] = (opt && opt.page) || current;
        pageParams[sizeField] = limit;
      }
      ...
      const item: any[] = [];
      resList.forEach((element: roleElement) => {
        const { createdAt, id, name, remark } = element;
        const newItem = {
      ...
      rawDataSourceRef.value = newRes;
      console.log('rawDataSourceRef.value :>>', newRes);
      ...
      if (resultTotal) {
        const currentTotalPage = Math.ceil(resultTotal / limit);
  ...
  function setTableData<T = Recordable>(values: T[]) {
    console.log('dataSourceRef.value :>>', dataSourceRef.value);
